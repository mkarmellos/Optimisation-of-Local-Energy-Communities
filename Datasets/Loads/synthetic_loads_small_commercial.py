# -*- coding: utf-8 -*-
"""
Synthetic Load Generator – Small Commercial
STL + Controlled Residual Bootstrap (FIXED)
"""
...
INPUT_CSV = "Typical Energy Load -small_commercial.csv"
TIME_COL  = None
LOAD_COL  = "Power [kW]"
DAYFIRST  = True
...
OUT_CSV     = "synthetic_profiles_small_commercial.csv"
OUT_METRICS = "synthetic_metrics_small_commercial.csv"
OUT_DIR     = "outputs/small commercial"
...
def synthesize(series, n_syn, seed):
    ...
    for k in range(1, n_syn + 1):
        ...
        # mild building-level amplitude heterogeneity (constant)
        amp = float(np.exp(rng_k.normal(0.0, 0.10)))

        # sample residuals with local scrambling
        syn_resid = rng_k.choice(resid_pool, size=len(idx), replace=True)

        # attenuate residuals to avoid dominance
        syn_resid *= 0.6
...
def plot_original(df_out: pd.DataFrame, outdir: str):
    ...
    df_out["load"].plot(ax=ax, color=PALETTE[0], linewidth=LINEWIDTH,
                        label="Sample Small Commercial Electricity Load")
